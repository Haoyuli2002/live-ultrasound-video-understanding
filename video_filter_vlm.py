...
超声视频 VLM 筛选器
====================
Stage 1: Qwen2-VL-2B (本地, MPS) → 逐帧分析 → 10个帧级JSON
Stage 2: DeepSeek API → 综合10帧结果 → 视频级最终判断

使用方法:
    # 在 notebook 中逐段运行，或直接命令行:
...
# Stage 1: VLM 逐帧分析的 Prompt (few-shot方式，避免小模型复读模板)
...
STAGE2_PROMPT_TEMPLATE = """你是超声视频数据集的质量控制专家。以下是一个视频中均匀采样10帧的VLM分析结果。
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num-frames", type=int, default=NUM_FRAMES, help="采样帧数 (默认10)")
